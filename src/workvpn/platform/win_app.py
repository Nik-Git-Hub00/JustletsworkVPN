...
from .win_backend import (
    ...
    ORANGE,
    RED,
    SINGBOX_ALIVE_CHECK_DELAY,
    SINGBOX_START_RETRIES,
    SINGBOX_START_RETRY_DELAY,
    SING_BOX,
    TOKEN_FILE,
    VPN_VERIFY_ERROR,
    ...
    create_tray_image,
    ELEVATED_WORKER_ARG,
    ElevatedSingBoxWorker,
    elevated_worker_main,
    fetch_public_ip,
    ICON_FILE,
...
def windows_app_icon() -> QIcon:
    icon = QIcon()
    # Small generated layers lose the shield details. Let Qt downscale the
    # clean 64 px source for the title bar and compact Windows taskbars.
    for size in (64, 128, 256):
        path = resource_path(f"assets/windows_icon_{size}.png")
        if path.is_file():
            icon.addFile(str(path), QSize(size, size))
    return icon if not icon.isNull() else QIcon(str(ICON_FILE))
...
def power_icon_asset(filename: str) -> QIcon:
    pixmap = pixmap_asset(filename)
    if pixmap.isNull():
        return QIcon()
    bounds = QRegion(pixmap.mask()).boundingRect()
    if bounds.isEmpty():
        return QIcon(pixmap)
    padding = max(2, round(min(bounds.width(), bounds.height()) * 0.025))
    bounds.adjust(-padding, -padding, padding, padding)
    return QIcon(pixmap.copy(bounds.intersected(pixmap.rect())))
...
class PowerButton(QPushButton):
    ...
    def set_state(self, state: str, enabled: bool):
        ...
        self.setIcon(power_icon_asset(f"power_button_{state}.png"))

    def set_diameter(self, diameter: int):
        visible_size = round(diameter * 0.68)
        self.setFixedSize(visible_size, visible_size)
        self.setIconSize(QSize(visible_size, visible_size))
...
class WorkVpnWindow(QMainWindow):
    def __init__(self):
        ...
        self.vpn_ip = None
        self.process = None
        self.elevated_worker = ElevatedSingBoxWorker(self.log_safe)
    ...
    def update_responsive_sizes(self):
        ...
        self.title_label.setFont(QFont("Segoe UI", title_size, QFont.Weight.Bold))
        self.subtitle_label.setFont(QFont("Segoe UI", subtitle_size))
        self.time_caption.setFont(QFont("Segoe UI", max(14, min(17, round(w * 0.03))), QFont.Weight.Bold))
        self.timer_label.setFont(QFont("Segoe UI", timer_size))
        self.status_label.setFont(QFont("Segoe UI", max(14, min(17, round(w * 0.03))), QFont.Weight.Bold))
    ...
    def log_panel_geometries(self):
        ...
        token_top = self.token_btn.mapTo(
            self.centralWidget(), QPoint(0, 0)
        ).y()
        opened_top = status_bottom + max(1, (token_top - status_bottom) // 2)
    ...
    def prepare_and_start_vpn(self):
        if not SING_BOX.exists():
            self.log_safe(tr("file_not_found", path=SING_BOX) + "\n")
            self.set_disconnected()
            self.bridge.error.emit(tr("error"), tr("file_not_found", path=SING_BOX))
            return
        ...
        self.launch_singbox_worker(before_ip)
    ...
    def verify_vpn_connection(self, before_ip, process):
    ...
    def launch_singbox_worker(self, before_ip=None):
        ...
        for attempt in range(1, SINGBOX_START_RETRIES + 1):
            ...
            try:
                process = self.elevated_worker
                process.start(CONFIG)
            except Exception as error:
                last_error = str(error)
                self.log_safe(tr("start_error", error=error))
                break
            ...
            self.bridge.error.emit(
                tr("error") if reason == tr("process_stopped") else tr("vpn_check_title"),
                VPN_START_ERROR if reason == tr("process_stopped") else VPN_VERIFY_ERROR,
            )
            return

        self.log_safe(tr("start_failed", error=last_error or tr("unknown_error")))
        self.set_disconnected()
        self.bridge.error.emit(tr("error"), VPN_START_ERROR)
    ...
    def stop_process_after_failed_verify(self, process=None):
        ...
        try:
            ...
            self.log_safe(tr("stop_error", error=error))
        if self.process is active_process:
            self.process = None

    def read_logs(self, process):
        try:
            ...
            self.log_safe(tr("read_log_error", error=error))

    def watch_process(self, process):
        code = process.wait()
        self.log_safe(tr("singbox_exit_code", code=code))
        if self.process is process:
            self.process = None
...
def main():
    if os.name != "nt":
        raise SystemExit("This client is intended for Windows.")
    if ELEVATED_WORKER_ARG in sys.argv:
        index = sys.argv.index(ELEVATED_WORKER_ARG)
        if len(sys.argv) <= index + 2:
            raise SystemExit(2)
        raise SystemExit(elevated_worker_main(int(sys.argv[index + 1]), sys.argv[index + 2]))
    ...
    app.setWindowIcon(windows_app_icon())
